...
from dataset import ClinicalDatasetProj
...
from utils_callback import SaveHyperparametersCallback, SavePredictionCallbackProj
...
def main(trainparams):
    ...
    #initialize the datasets & dataloader
    dataset_train = ClinicalDatasetProj(**trainparams['dataset'], **trainparams['dataset_train'])
    dataset_val = ClinicalDatasetProj(**trainparams['dataset'], **trainparams['dataset_val'])
    ...
    # ----- init model -----
    litmodel = Neighbor2NeighborModule(network=base_network, 
    ...
    #init image logger
    prediction_callback = SavePredictionCallbackProj(validation_dataset=dataset_val, output_dir=save_path + f"/lightning_logs/version_{tblogger.version}/predictions/") #saves an image of the current model prediction of the val set
